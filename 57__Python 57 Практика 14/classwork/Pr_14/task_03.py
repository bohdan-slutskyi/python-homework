...
# Генераторная функция: бесконечно выдаёт случайные имя + фамилию
def random_names():
    while True:
        yield f"{choice(first_names)} {choice(last_names)}"  # каждое значение — одна пара
...
if __name__ == "__main__":
    ...
    for _ in range(5):
        print(next(gen))  # next() получает очередное значение из генератора
